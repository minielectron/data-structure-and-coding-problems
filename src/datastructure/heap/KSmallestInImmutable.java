...
public class KSmallestInImmutable {
// K Largest Elements In An Immutable Max-Heap
// Given a max-heap in array representation, return the k largest elements in
// the heap without performing explicit removals from the max-heap (the heap is immutable).
    ...
    public List<Integer> kLargestInImmutableHeap(int[] immutableHeap, int k) {
        ...
        /*
         * We will do k iterations and add the left and right child of each item touched
         * to the 'maxCandidates' max heap. Since this is a max heap, we know that both
         * children will be <= the parent item.
         *
         * If that item every gets added as an item in the max k items, it "unlocks" its
         * children to be now under consideration since we "make room" for them not to
         * be dominated by their parent in value.
         */
        for (int i = 0; i < k; i++) {
     ...
     * implement a custom compare function to reverse this to have max heap
     * behaviour
     */
